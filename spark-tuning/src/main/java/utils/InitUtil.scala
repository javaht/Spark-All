...
import bean.{School, Student}

object InitUtil {


  def main( args: Array[String] ): Unit = {
    val sparkConf = new SparkConf().setAppName("InitData").setMaster("local[*]") //TODO 要打包提交集群执行，注释掉
  ...
  def initSparkSession( sparkConf: SparkConf ): SparkSession = {
    ...
    // TODO 改成自己的地址
    ssc.hadoopConfiguration.set("fs.defaultFS", "hdfs://192.168.20.62:8020")
    sparkSession
  }
